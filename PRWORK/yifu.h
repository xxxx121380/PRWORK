...
using namespace std;
void outyifu(string path);
void outyifu(string path)
{
	vector<int> bluelevel, yellowlevel;
	string bluetype = path;
	bluetype.append("\\词缀\\蓝\\type\\blue_type_26.txt");
	string yellowtype = path;
	yellowtype.append("\\词缀\\黄\\type\\yellow_type_26.txt");
	ifstream inFile("equip\\衣服.csv", ios::in);
	if (!inFile)
	{
		cout << "打开衣服文件失败！" << endl;
		exit(-1);
	...
	while (getline(inFile, line))//getline(inFile, line)表示按行读取CSV文件中的数据
	{
		...
		if (sign0 == 0)
			outFile << _T("      - \'&f◈&l 类型: 中型\'") << endl;
		else if (sign0 == 0.1)
			outFile << _T("      - \'&f◈&l 类型: 轻型\'") << endl;
		else outFile << _T("      - \'&f◈&l 类型: 重型\'") << endl;
		...
		outFile << "  " << string2wstring(name) << _T("_基础_移动速度:") << endl;
		outFile << "    type: attribute" << endl;
		outFile << "    data:" << endl;
		outFile << _T("      id: 移动速度") << endl;
		outFile << "      min: " << string2wstring(sign) << endl;
		outFile << "      max: {min}" << endl;
		outFile << "      sign: true" << endl;
		outFile << "      float: 0" << endl;
		outFile << "      order: 1" << endl;
		outFile << "      display:" << endl;
		outFile << _T("      - \'&a▣ 移动速度: &9{attribute.streng.summary}\'") << endl;
		outFile.close();
		...
		outFile1 << _T("template: 白装衣服模板") << endl;
		outFile1 << "damage: 0" << endl;
		outFile1 << "need-class: []" << endl;
		outFile1 << _T("equip-type: 衣服") << endl;
		...
		outFile1 << "  ks: \'" << string2wstring(hole) << "\'" << endl;
		outFile1 << "sell: " << endl;
		...
		outFile1 << "type: iron_chestplate" << endl;
		outFile1 << "gem:" << endl;
		outFile1 << "  driller: true" << endl;
		outFile1 << "hide: 63" << endl;

		outFile1 << "need-level: \'" << equiplevel << '\'' << endl;
		outFile1 << _T("template-not-appraisal: 随机衣服") << endl;
		...
		outFile3 << _T("template: 黄装衣服模板") << endl;
		outFile3 << "damage: 0" << endl;
		outFile3 << "need-class: []" << endl;
		outFile3 << _T("equip-type: 衣服") << endl;
		...
		outFile3 << _T("  random: 黄_类别_26_等级_") << string2wstring(to_string(truelevel)) << endl;
		outFile3 << "type: iron_chestplate" << endl;
		outFile3 << "gem:" << endl;
		outFile3 << "  driller: false" << endl;
		outFile3 << "hide: 63" << endl;

		outFile3 << "need-level: \'" << equiplevel << '\'' << endl;
		outFile3 << _T("template-not-appraisal: 随机衣服") << endl;
		...
		outFile2 << _T("template: 蓝装衣服模板") << endl;
		outFile2 << "damage: 0" << endl;
		outFile2 << "need-class: []" << endl;
		outFile2 << _T("equip-type: 衣服") << endl;
		...
		outFile2 << "sell: " << endl;
		outFile2 << "  ks: \'";
		if (atoi(hole.c_str()) == 2)
			outFile2 << 1 << '\'' << endl;
		else if (atoi(hole.c_str()) == 1)
			outFile2 << 0 << '\'' << endl;
		else if (atoi(hole.c_str()) == 3)
			outFile2 << 2 << '\'' << endl;
		else if (atoi(hole.c_str()) == 4)
			outFile2 << 3 << '\'' << endl;
		...
		outFile2 << _T("  random: 蓝_类别_26_等级_") << string2wstring(to_string(truelevel)) << endl;
		outFile2 << "type: iron_chestplate" << endl;
		outFile2 << "gem:" << endl;
		outFile2 << "  driller: true" << endl;
		outFile2 << "hide: 63" << endl;

		outFile2 << "need-level: \'" << equiplevel << '\'' << endl;
		outFile2 << _T("template-not-appraisal: 随机衣服") << endl;
